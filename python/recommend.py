...
def find_movieId(_input_movies, _movies_df):
    ...
    for movie in _input_movies:
        index = _movies_df.loc[_movies_df["original_title"]
                               == movie, 'id'].values[0]
...
def do_apriori(_input_movies, _movies_df, _ratings_df):
    ...
    df = pd.merge(
        _ratings_df, _movies_df[['id', 'title']], left_on='movieId', right_on='id')
    ...
    frequent_items = apriori(df_pivot, min_support=0.03, use_colnames=True)
...
def main(input_movies):
    ...
    # Read csv files
    print(input_movies)
    movies_df = pd.read_csv('python/movie_detail.csv')
